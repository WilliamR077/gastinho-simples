-- POST-TOOLS G1-C1: make atomic shared-group creation available.
--
-- The function derives identity from auth.uid(), serializes quota checks per
-- user, and creates the group and owner membership in the transaction already
-- provided by the RPC statement. Table grants intentionally remain unchanged
-- for a rollout compatible with the frontend version currently in production.

CREATE FUNCTION public.create_shared_group_atomic(
  p_name text,
...
BEGIN
  ...
  -- Serialize only creations by this authenticated user. The lock is released
  -- automatically at transaction end and prevents concurrent quota bypass.
  ...
  BEGIN
    -- Preserve the current six-character alphabet. Six random bytes map
    -- without modulo bias because the alphabet contains exactly 32 symbols.
    FOR v_attempt IN 1..5 LOOP
      v_random_bytes := public.gen_random_bytes(6);
      v_invite_code := '';
  ...
  EXCEPTION
    WHEN SQLSTATE 'P0001' THEN
      RAISE;
    WHEN OTHERS THEN
      -- The exception block is a PostgreSQL subtransaction: both inserts are
      -- rolled back before this stable, non-sensitive error reaches the client.
      RAISE EXCEPTION USING
        ERRCODE = 'P0001',
