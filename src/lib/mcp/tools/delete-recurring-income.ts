...
export default defineTool({
  ...
  description:
    "Exclui permanentemente somente um template mensal de receita pertencente à conta autenticada. Não exclui receitas nem ocorrências reais.",
  ...
  handler: async (rawInput, ctx) => {
    ...
    if (
      !recognizable ||
      !recurringIncomeViewSchema.safeParse(recognizable).success
    ) {
    ...
    const deletedTemplate = recurringIncomeView(
      deleteResult.data as RecurringIncomeWriteRow,
    );
    if (
      !deletedTemplate ||
      !recurringIncomeViewSchema.safeParse(deletedTemplate).success
    ) {
      return mcpError("INVALID_DATA");
    }
